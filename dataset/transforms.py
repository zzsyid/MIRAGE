...
def pad_stream_to_length(stream, target_len, dim=0):
    curr_len = stream.shape[dim]
    if curr_len >= target_len:
        return stream
    pad_count = target_len - curr_len
    last = stream.narrow(dim, curr_len - 1, 1)
    pad = last.repeat(*[pad_count if i == dim else 1 for i in range(stream.ndim)])
    return torch.cat([stream, pad], dim=dim)
...
class TemporalCropAndOffset(torch.nn.Module):
    ...
    def _sample_feasible_offset(self, v_start_max_sec, is_oos=None):
        max_feasible_off_sec = min(self.max_off_sec, v_start_max_sec)
        if self.offset_type == 'grid':
            feasible_offsets = [off for off in self.class_grid.tolist() if abs(off) <= (max_feasible_off_sec + 1e-6)]
            assert len(feasible_offsets) > 0, f'No feasible grid offsets for max start {v_start_max_sec}'
            return random.choice(feasible_offsets)
        elif self.offset_type == 'uniform':
            return random.uniform(-max_feasible_off_sec, max_feasible_off_sec)
        elif self.offset_type == 'uniform_binary':
            if is_oos:
                offset_sec = random.uniform(-max_feasible_off_sec, max_feasible_off_sec)
                while self.itu_t_range[0] <= offset_sec <= self.itu_t_range[1]:
                    offset_sec = random.uniform(-max_feasible_off_sec, max_feasible_off_sec)
                return offset_sec
            return self.ins_dist.sample().item()
        raise NotImplementedError(f'Unknown offset type: {self.offset_type}')
    ...
    def forward(self, item):
        ...
        max_video_pad = max(2, int(round(v_fps * 0.08)))
        max_audio_pad = max_video_pad * max(1, int(round(a_fps / v_fps)))

        if v_len_frames < v_crop_len_frames:
            deficit = v_crop_len_frames - v_len_frames
            assert deficit <= max_video_pad, f'{v_len_frames} {v_crop_len_frames} {v_fps} @ {item["path"]}'
            vid = pad_stream_to_length(vid, v_crop_len_frames, dim=0)
            v_len_frames = vid.shape[0]
        if a_len_frames < a_crop_len_frames:
            deficit = a_crop_len_frames - a_len_frames
            assert deficit <= max_audio_pad, f'{a_len_frames} {a_crop_len_frames} {a_fps} @ {item["path"]}'
            aud = pad_stream_to_length(aud, a_crop_len_frames, dim=0)
            a_len_frames = aud.shape[0]
        ...
        if self.do_offset:
            ...
            if offset_sec is None and v_start_i_sec is None:
                ...
                if self.offset_type == 'grid':
                    offset_sec = self._sample_feasible_offset(v_start_max_sec)
                elif self.offset_type == 'uniform':
                    offset_sec = self._sample_feasible_offset(v_start_max_sec)
                elif self.offset_type == 'uniform_binary':
                    ...
                    offset_sec = self._sample_feasible_offset(v_start_max_sec, is_oos=is_oos)
