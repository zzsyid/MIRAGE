    ...
    def vizualize_input(self, vid: torch.Tensor, aud: torch.Tensor, batch, global_iter: int, phase: str,
        ...
        if not hasattr(self, 'griffinlim'):
            if is_input_segmented:
                ...
                inverse_mel_kwargs = dict(n_stft=n_fft // 2 + 1, n_mels=F, sample_rate=afps)
                if 'tolerance_change' in inspect.signature(InverseMelScale.__init__).parameters:
                    inverse_mel_kwargs['tolerance_change'] = 0.00001
                self.griffinlim = torchvision.transforms.Compose([
                    InverseMelScale(**inverse_mel_kwargs),
